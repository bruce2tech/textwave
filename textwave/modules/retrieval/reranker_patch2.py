...
class Reranker:
    ...
    def __init__(self, type: str = "tfidf", model_name: str = "cross-encoder/ms-marco-MiniLM-L-6-v2"):
        ...
        # Eagerly initialize so unit-test patches take effect
        from transformers import AutoTokenizer, AutoModelForSequenceClassification
    ...
    def cross_encoder_rerank(
        ...
        **_: object,
        ...
        order = np.argsort(-logits)          # DESCENDING by score
        top = order[:k]
    ...
    def sequential_rerank(
            ...
            k2: int,
            *,
            distance_metric="cosine",  # accepted for API compatibility; unused
            **_: object,
        ) -> tuple[list[str], list[int], list[float]]:
            """
            Two-phase pipeline:
